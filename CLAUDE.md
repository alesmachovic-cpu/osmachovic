...
- **Vercel auto-deploy z `dev` branch je ROZBITÝ.** Push do `dev` neaktivuje deploy na dev.amgd.sk (overené 2026-05-23: 7+ commitov push-nutých, žiadny deploy 4+ hodín). Po každom push-i ručne spustiť `vercel deploy --prod --yes` z fresh clone main, alebo Alešovi explicitne povedať že treba ručne deploy. **NIKDY nehovor "uvidíš na dev.amgd.sk po 1-2 min"** — to nie je pravda kým sa auto-deploy nevyrieši. Príčina zatiaľ neznáma (GitHub webhook? branch config? Vercel post-incident 20.5. zmena?).
