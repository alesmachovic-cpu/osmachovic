...
- **Direct-to-main hotfix MUSÍ byť okamžite back-merge-nutý do `dev`.** Inak `dev` zaostane na security a ďalší `dev→main` deploy hrozí (zdanlivou) regresiou. Lekcia: 9.6 prod-lockdown hotfixy (requireUser na 17 routes, S3 cross-tenant guard v `klient-udalosti`) išli priamo na `main` počas zatvárania anon dier, ale **neback-merge-li do `dev`** → `dev` ostalo 17 routes bez auth. Pred ďalším prod deployom (12.6, G33 erasure) to v `git diff b7245aa..origin/dev` vyzeralo ako 20 odstránených `requireUser` = regresia.
- **`git diff` medzi vetvami NIE JE čo merge spraví.** Diff porovnáva dva koncové body; merge je 3-way (zohľadní merge-base). Pred prod merge-om VŽDY over SKUTOČNÝ výsledok: `git merge-tree --write-tree A B` → tree, potom porovnaj blob-hashe `git ls-tree <tree> <file>` vs `git rev-parse A:<file>` / `B:<file>`. Ak dev súbor needitoval od merge-base (0 commitov), merge zachová prod verziu — žiadna regresia. Toto rozlíšenie zachránilo G33 deploy od falošného STOP-u aj od slepého merge-u.
- **Pre security-citlivý prod deploy preferuj cielený deploy pred plným `dev→main` merge.** Cherry-pick len relevantné súbory cez worktree (`git worktree add`, branch, push `branch:main`) → menší diff, prod baseline 1:1 nedotknutý, ľahší rollback (revert 1 commit). G33 = 3 súbory cielene namiesto 19-commit merge.
- **Po každom direct-to-main hotfixe ihneď back-merge do `dev`** (alebo eviduj ako dlh) — inak `dev≠prod` divergencia rastie a každý ďalší merge je mínové pole.
