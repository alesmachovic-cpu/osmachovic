 ...
 * Otvorí modal kde maklér zadá email klienta → POST /api/sign/request channel=email
 * → klient dostane email s linkom + OTP na podpis (cez Resend, zadarmo)
 *
 * SMS variant je pripravený v API + lib/sms.ts ale UI ho zatiaľ neukazuje
 * (čaká sa kým bude SMS provider nakonfigurovaný — Twilio/Smslogic).
 */
            ...
            {!result && (
              <>
                <p style={{ fontSize: "13px", color: "var(--text-secondary)", margin: "0 0 16px", lineHeight: 1.5 }}>
                  Zadaj email klienta. Pošleme mu link na verejnú stránku s 6-cifrovým overovacím kódom (platí 15 min).
                </p>
                <input
                  type="email"
                  value={email}
                ...
                <button
                  ...
                  {loading ? "Odosielam..." : "📤 Poslať email"}
                </button>
              </>
            )}
            ...
            {result && !result.manual && (
              <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
                <div style={{
                  ...
                  ✓ Email odoslaný na {email}
                </div>
                <p style={{ fontSize: "12px", color: "var(--text-secondary)", margin: 0, lineHeight: 1.5 }}>
                  Klient otvorí link v emaile, zadá kód a podpíše. Tu uvidíš keď bude hotovo
                ...
                <button onClick={copyLink} style={{
                  ...
                  📋 Skopírovať link (pre prípad že email neprišiel)
                </button>
              </div>
            )}
            ...
            {result && result.manual && (
              <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
                <div style={{
                  ...
                  ⚠️ Email sa nepodarilo odoslať. Pošli klientovi tieto údaje cez WhatsApp / volaním:
                </div>
                <div style={{
