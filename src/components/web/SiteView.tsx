 ...
 * Dáta prichádzajú z web_sites (published alebo draft pre náhľad).
 * Štýly sú scoped pod `.mw` (viď src/lib/web/styles.ts).
 */
import type { CSSProperties } from "react";
import type { MaklerData, ManazerData, Nastavenia, SiteData, WebSite } from "@/lib/web/types";
import { initials, sectionOn, telFrom } from "@/lib/web/types";
import { MAKLER_CSS, MANAZER_CSS } from "@/lib/web/styles";
...
function MaklerSite({ site, data: b, refs, nastavenia: BR, preview, dark }: SiteViewProps & { data: MaklerData }) {
  ...
  const mUrl = manager ? `/web/${manager.slug}` : "#";
              ...
              {team.map(r => {
                ...
                return (
                  <div key={r.id} className="member">
                    ...
                    <div className="areas-s">{m.areas.slice(0, 3).join(" · ")}</div>
                    <div className="go">
                      {r.isPublished && <a className="btn btn-primary" href={`/web/${r.slug}`}>Web makléra →</a>}
...
function ManazerSite({ site, data: M, refs, nastavenia: BR, preview, dark }: SiteViewProps & { data: ManazerData }) {
  ...
  const teamUrl = team.length ? `/web/${team[0].slug}` : BR.url;
