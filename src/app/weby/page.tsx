...
import {
  PALETTES, SKIES, SECTIONS_MAKLER, SECTIONS_MANAZER, sectionOn, slugify, telFrom,
  type Listing, type MaklerData, type ManazerData, type Nastavenia, type PaletteId, type SectionId, type SiteData, type SkyId, type WebDopyt, type WebSite,
} from "@/lib/web/types";

type Site = WebSite & { canEdit: boolean };
type View = { kind: "site"; id: string } | { kind: "nastavenia" } | { kind: "dopyty" };
...
async function uploadImage(file: File, siteId: string): Promise<string> {
  ...
  const r = await fetch("/api/web/upload", { method: "POST", body: fd, credentials: "include" });
  const d = await r.json().catch(() => ({}));
    ...
    <div className={compact ? "" : "wa-f"}>
      ...
      <div className="wa-photo">
        ...
        <div className="wa-pin">
          <input value={value || ""} placeholder="URL alebo /web/nazov.jpg" onChange={e => onChange(e.target.value)} />
...
function SiteEditor({ site, isManager, accounts, onDraft, onMeta, onPublish, onDelete, onDuplicate, saveState }: {
  ...
  onDraft: (d: SiteData) => void; onMeta: (m: { slug?: string; user_id?: string | null }) => Promise<void>;
  ...
  return (
    <div className={"wa-editor" + (ro ? " wa-ro" : "")}>
      ...
      <div className="wa-status">
        ...
        {site.published && <a className="wa-link" href={`/web/${site.slug}`} target="_blank" rel="noreferrer">/web/{site.slug} ↗</a>}
      </div>
      ...
      {ro && <div className="wa-note">Tento web môžeš len prezerať. Upravovať ho môže priradený maklér alebo manažér.</div>}
      ...
      {isManager && (
        <>
          <H2>Adresa a priradenie</H2>
          <div className="wa-row2">
            <div className="wa-f">
              <label>Verejná adresa</label>
              <div className="wa-slug"><span>/web/</span><input value={slug} onChange={e => setSlug(e.target.value)} onBlur={async () => { if (slug !== site.slug) { try { setMetaErr(""); await onMeta({ slug }); setSlug(slugify(slug)); } catch (e) { setMetaErr((e as Error).message); setSlug(site.slug); } } }} /></div>
              {metaErr && <div className="wa-err">{metaErr}</div>}
              <div className="wa-hint">Zmena adresy zneplatní staré odkazy.</div>
            </div>
            <div className="wa-f">
              <label>Priradený používateľ CRM</label>
              <select value={site.user_id || ""} onChange={e => onMeta({ user_id: e.target.value || null })}>
                <option value="">— nikto —</option>
                {accounts.map(a => <option key={a.id} value={a.id}>{a.name}</option>)}
              </select>
              <div className="wa-hint">Môže si web upravovať sám a dostáva dopyty z formulára (push + e‑mail).</div>
            </div>
          </div>
        </>
      )}
...
function Dopyty({ sites }: { sites: Site[] }) {
  ...
  useEffect(() => {
    api<{ dopyty: WebDopyt[] }>("/api/web/dopyty").then(d => setRows(d.dopyty)).catch(e => setErr(e.message));
  }, [tick]);
            ...
            <tbody>
              {rows.map(r => (
                <tr key={r.id} className={r.vybavene ? "done" : ""}>
                  ...
                  <td>{r.data.kedy || "—"}</td>
                  <td><input type="checkbox" checked={r.vybavene} onChange={async e => { const v = e.target.checked; setRows(rs => rs!.map(x => x.id === r.id ? { ...x, vybavene: v } : x)); await api("/api/web/dopyty", { method: "PATCH", body: JSON.stringify({ id: r.id, vybavene: v }) }).catch(() => load()); }} /></td>
                </tr>
              ))}
            </tbody>
          </table>
...
function WebyAdmin() {
  ...
  useEffect(() => {
    api<{ sites: Site[]; nastavenia: Nastavenia; isManager: boolean }>("/api/web/sites")
      .then(d => {
        setSites(d.sites); setNastavenia(d.nastavenia); setIsManager(d.isManager);
        setView(v => (v.kind === "site" && !v.id) ? { kind: "site", id: (d.sites.find(s => s.user_id === userId) || d.sites[0])?.id || "" } : v);
      })
  ...
  function onDraft(id: string, draft: SiteData) {
    ...
    timers.current[id] = setTimeout(async () => {
      ...
      try {
        await api(`/api/web/sites/${id}`, { method: "PUT", body: JSON.stringify({ draft: d }) });
  ...
  async function onMeta(id: string, m: { slug?: string; user_id?: string | null; poradie?: number }) {
    const d = await api<{ site: Site }>(`/api/web/sites/${id}`, { method: "PUT", body: JSON.stringify(m) });
    setSites(ss => ss.map(s => s.id === id ? { ...s, ...d.site, draft: s.draft } : s));
  }
  async function onPublish(id: string, unpublish?: boolean) {
    clearTimeout(timers.current[id]);
    const d = pending.current[id];
    if (d) { delete pending.current[id]; await api(`/api/web/sites/${id}`, { method: "PUT", body: JSON.stringify({ draft: d }) }); }
    const r = await api<{ site: Site }>(`/api/web/sites/${id}/publish`, { method: "POST", body: JSON.stringify({ unpublish: !!unpublish }) });
    setSites(ss => ss.map(s => s.id === id ? { ...s, ...r.site } : s));
    setPrevKey(k => k + 1);
  }
  async function addSite(copyOf?: string) {
    const r = await api<{ site: Site }>("/api/web/sites", { method: "POST", body: JSON.stringify({ copyOf }) });
  ...
  async function delSite(s: Site) {
    ...
    await api(`/api/web/sites/${s.id}`, { method: "DELETE" });
    setSites(ss => ss.filter(x => x.id !== s.id));
    setView({ kind: "site", id: sites.find(x => x.id !== s.id)?.id || "" });
  }
  async function reorder(ids: string[]) {
    await Promise.all(ids.map((id, i) => api(`/api/web/sites/${id}`, { method: "PUT", body: JSON.stringify({ poradie: i + 1 }) })));
    setSites(ss => ss.map(s => { const i = ids.indexOf(s.id); return i >= 0 ? { ...s, poradie: i + 1 } : s; }));
    setPrevKey(k => k + 1);
  }
  async function saveNastavenia(n: Nastavenia) {
    const d = await api<{ nastavenia: Nastavenia }>("/api/web/nastavenia", { method: "PUT", body: JSON.stringify(n) });
    setNastavenia(d.nastavenia); setPrevKey(k => k + 1);
  }

  const previewSrc = current ? `/web/nahlad/${current.id}?v=${prevKey}${prevDark ? "&dark=1" : ""}` : "";
    ...
    <div className={"wa" + (mobilePrev ? " show-prev" : "")}>
      ...
      <section className="wa-main">
        ...
        {view.kind === "site" && current && (
          <SiteEditor key={current.id} site={current} isManager={isManager} accounts={accounts.map(a => ({ id: a.id, name: a.name }))}
        ...
        {view.kind === "site" && !current && !loadErr && <div className="wa-note" style={{ margin: 20 }}>{sites.length ? "Vyber web vľavo." : "Načítavam…"}</div>}
...
/* ─────────────────────────── Štýly adminu ─────────────────────────── */
const ADMIN_CSS = `
.wa{display:grid;grid-template-columns:230px minmax(380px,520px) 1fr;gap:0;height:calc(100vh - 64px - 48px);margin:-24px -28px;border-top:1px solid var(--border);font-size:14px;color:var(--text-primary)}
.wa-side{background:var(--bg-surface);border-right:1px solid var(--border);overflow:auto;padding-bottom:20px}
...
.wa-table table{width:100%;border-collapse:collapse;font-size:13px}
...
@media (max-width:768px){
  .wa{grid-template-columns:1fr;grid-template-rows:auto 1fr;height:auto;margin:-16px}
