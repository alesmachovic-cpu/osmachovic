...
export async function POST(req: NextRequest) {
  ...
  const { data, error } = await supabase.from("prehlad_zaznamy").insert(payload).select().single();
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
  return NextResponse.json(data);
}
...
export async function PATCH(req: NextRequest) {
  ...
  const { data, error } = await supabase.from("prehlad_zaznamy").update(rest).eq("id", id).select().single();
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
  // sync s faktúrou
  if (rest.zaplatene !== undefined && data?.faktura_id) {
    await supabase.from("faktury").update({
...
export async function DELETE(req: NextRequest) {
  ...
  if (!id) return NextResponse.json({ error: "id required" }, { status: 400 });
  const { error } = await supabase.from("prehlad_zaznamy").delete().eq("id", id);
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
