...
export async function POST(req: NextRequest) {
  try {
    ...
    if (telefon) {
      ...
      const { data: dup } = await supabase.from('klienti').select('id, meno, priezvisko').or('telefon.eq.' + telefon + ',telefon.eq.' + clean).neq('id', klient_id || '00000000-0000-0000-0000-000000000000');
    ...
    if (email) {
      const { data: dup } = await supabase.from('klienti').select('id, meno, priezvisko').eq('email', email.toLowerCase()).neq('id', klient_id || '00000000-0000-0000-0000-000000000000');
    ...
    if (kolize.length > 0) await supabase.from('kolizny_log').insert(kolize.map(k => ({ typ_kolizie: k.typ, zavaznost: k.zavaznost, popis: k.sprava, meta_data: k.data, stav: 'nova', vytvorene: new Date().toISOString() })));
