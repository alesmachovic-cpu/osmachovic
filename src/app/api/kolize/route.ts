...
export async function GET(req: NextRequest) {
  try {
    ...
    let query = supabase.from('kolizny_log').select('*').order('vytvorene', { ascending: false });
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    const { data, error } = await supabase.from('kolizny_log').update({ stav, poznamka: poznamka || null, aktualizovane: new Date().toISOString() }).eq('id', id).select().single();
    if (error) throw error;
