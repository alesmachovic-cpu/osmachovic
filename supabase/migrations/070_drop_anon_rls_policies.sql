-- Migration 070 — DROP nedbalivé anon RLS polices
...
--
-- ROLLBACK: viď supabase/migrations/070_rollback_anon_policies.sql

-- ═══ 1) monitor_filtre — bola plne otvorená pre všetky role (žiadne TO klauzule) ═══
...
-- ═══ 3) klient_udalosti, klienti_history (migrácia 047, 026) ═══
...
-- ═══ 4) Biz intelligence tabuľky (migrácie 036-041) ═══
...
-- ═══ 5) audit_log — bola USING(true) bez TO klauzuly = otvorená pre všetkých ═══
-- Replace s service-role-only read policy
DROP POLICY IF EXISTS "audit_log_read_admin" ON audit_log;
CREATE POLICY "audit_log_read_service" ON audit_log
  FOR SELECT USING (auth.role() = 'service_role');

-- ═══ 6) users — google_access_token, google_refresh_token musia byť service-only ═══
-- Bola read_users USING(true) — t.j. anon mohol vidieť šifrované OAuth tokeny
-- (aj keď sú šifrované, leak by mohol pomôcť pri offline útoku na key)
