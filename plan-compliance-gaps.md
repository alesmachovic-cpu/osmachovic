...
- **G31 — Faktúrne číslovanie = per DODÁVATEĽ (maklér), NIE per firma — dev migr. 075 je regresia:** OPRAVENÉ POSÚDENIE (pôvodne mylne „má byť per firma"). Model: dodávateľ faktúry = maklér (SZČO, vlastné IČO, makler_dodavatel migr. 029 + dodavatel_snapshot migr. 100); odberateľ = Vianema. Poradové číslo (§ 74/1/b zák. 222/2004, § 8 zák. 431/2002) je sekvenčné a unikátne **v rámci radu dodávateľa** → rôzni makléri (rôzne IČO) môžu mať to isté číslo legitímne (3× FA20260001 ≠ duplicita). **PROD index `(user_id, cislo_faktury)` (migr. 032) = SPRÁVNE, zachovať.** **DEV migr. 075 zmenila na `(company_id, cislo_faktury)` = CHYBA — zlučuje samostatné dodávateľské rady → Financie vráti späť na per-dodávateľ.** Stále platí v rámci 1 makléra: unique + sekvenčné + atomické (retry 23505) + nemenné po vystavení. Žiadne mazanie/prečíslovanie netreba.
