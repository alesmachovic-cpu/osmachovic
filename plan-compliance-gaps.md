...
- **G31 — Faktúrne číslovanie = per DODÁVATEĽ/MAKLÉR (✅ POTVRDENÉ CEO 2026-06-09):** Každý maklér má vlastnú firmu/IČO a fakturuje províziu Vianeme ako samostatný dodávateľ (odberateľ = Vianema; schéma: makler_dodavatel migr. 029 per user + dodavatel_snapshot migr. 100). ⇒ účtovná jednotka pre číslovanie = MAKLÉR → poradové číslo unikátne v rámci radu DODÁVATEĽA (§ 74/1/b zák. 222/2004, § 8 zák. 431/2002). **3× FA20260001 od 3 maklérov = NIE duplicita** (3 samostatné rady/IČO). **PROD index `(user_id, cislo_faktury)` (migr. 032) je SPRÁVNY — zachovať.** **Dev migr. 075 `(company_id, cislo_faktury)` = REGRESIA → Financie revert (fast-follow).** Žiadne prečíslovanie/mazanie faktúr. (Pozn.: pôvodné posúdenie „per firma" bolo opravené — vychádzalo z mylného predpokladu, že vystavovateľ je Vianema.) Poradové číslo (§ 74/1/b zák. 222/2004, § 8 zák. 431/2002) je sekvenčné a unikátne **v rámci radu dodávateľa** → rôzni makléri (rôzne IČO) môžu mať to isté číslo legitímne (3× FA20260001 ≠ duplicita). **PROD index `(user_id, cislo_faktury)` (migr. 032) = SPRÁVNE, zachovať.** **DEV migr. 075 zmenila na `(company_id, cislo_faktury)` = CHYBA — zlučuje samostatné dodávateľské rady → Financie vráti späť na per-dodávateľ.** Stále platí v rámci 1 makléra: unique + sekvenčné + atomické (retry 23505) + nemenné po vystavení. Žiadne mazanie/prečíslovanie netreba.
