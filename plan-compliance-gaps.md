...
- **G31 — Faktúrne číslovanie = per DODÁVATEĽ — scope závisí od IČO (⏳ čaká na potvrdenie CEO):** Rozhoduje, čie IČO je dodávateľom na faktúre. **(i) Ak každý maklér = vlastné IČO** (čo schéma indikuje: makler_dodavatel migr. 029 PK=user_id s vlastnými polami IČO/DIČ/IBAN + dodavatel_snapshot migr. 100; odberateľ = Vianema) → účtovná jednotka = maklér → **per-MAKLÉR rad správny (prod migr. 032), dev 075 (per-company) je regresia, 3× FA20260001 ≠ duplicita.** **(ii) Ak všetci pod jedným Vianema IČO** → per-FIRMA rad, dev 075 správny, 3× FA = reálna duplicita. **Pôvodné posúdenie „má byť per firma" som opravil — bolo na mylnom predpoklade, že vystavovateľ je Vianema.** Definitívne rozhodne skutočné IČO (porovnať dodavatel_snapshot->>ico na tých 3 faktúrach / potvrdí CEO). Nemeniť nič na faktúrach kým nepotvrdené. Poradové číslo (§ 74/1/b zák. 222/2004, § 8 zák. 431/2002) je sekvenčné a unikátne **v rámci radu dodávateľa** → rôzni makléri (rôzne IČO) môžu mať to isté číslo legitímne (3× FA20260001 ≠ duplicita). **PROD index `(user_id, cislo_faktury)` (migr. 032) = SPRÁVNE, zachovať.** **DEV migr. 075 zmenila na `(company_id, cislo_faktury)` = CHYBA — zlučuje samostatné dodávateľské rady → Financie vráti späť na per-dodávateľ.** Stále platí v rámci 1 makléra: unique + sekvenčné + atomické (retry 23505) + nemenné po vystavení. Žiadne mazanie/prečíslovanie netreba.
