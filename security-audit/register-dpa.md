...
| **Verzia** | 1.0 |
| **Dátum** | 7. jún 2026 |
| **Naposledy zosúladené s kódom** | 2026-06-07 |
...
**Legenda stavu subprocesora (zhodná s RoPA):** `aktívny` / `kód pripravený — NEAKTÍVNY na prode (env overí Bezpecnost)` / `zámerne neuvedený v zásadách`.
...
| 4 | **Google LLC** | Drive (dokumenty klientov), Gmail (komunikácia), Calendar (termíny), Gemini (neosobné analýzy), push (FCM) | USA | EU-US DPF + SCC | Google Cloud/Workspace DPA (+ Google APIs Terms pre Gemini/FCM) — **uložiť odkaz** | ✅ áno | Aleš Machovič |
...
| 6 | **OpenAI (OpenAI, L.L.C.)** | AI fallback **len na NEosobné údaje** (copywriting, trhová/cenová analýza, parse fallback) — GPT-4o | USA | EU-US DPF (Data Processing Addendum) | DPA: openai.com/policies/data-processing-addendum — **uložiť DPA** | ✅ áno (v2.2) | Aleš Machovič + Compliance |
| 7 | **Stripe Inc.** | Spracovanie platieb predplatného CRM (billing SaaS — vzťah Machovic↔RK) | USA (EÚ entita Stripe Payments Europe pre EU zákazníkov) | EU-US DPF / SCC | Stripe DPA: stripe.com/legal/dpa — **uložiť DPA** | ⚪ kód pripravený — NEAKTÍVNY na prode (env overí Bezpecnost) · *env na prode: nie — overí Bezpecnost (Vercel)* | Aleš Machovič |
| 8 | **Twilio Inc.** (default SMS) / **Smslogic s. r. o.** (alternatíva) | Doručenie OTP/SMS pri elektronickom podpise (telefónne číslo príjemcu) | Twilio — USA / Smslogic — SR (EÚ) | Twilio: EU-US DPF + SCC / Smslogic: EHP (bez prenosu) | Twilio DPA: twilio.com/legal/data-protection-addendum — **uložiť DPA**; pri Smslogic uzavrieť čl. 28 zmluvu (SK) | ⚪ kód pripravený — NEAKTÍVNY na prode (env overí Bezpecnost) · *env na prode: nie — overí Bezpecnost (Vercel)* | Aleš Machovič |
| 9 | **ScrapingBee SAS** | SaaS scraping konkurenčných inzerátov (obsah stránok vrátane PII predajcu prechádza v prenose, neukladá sa) | Francúzsko/EÚ (over reálnu lokalitu spracovania) | EHP, príp. SCC ak spracovanie mimo EÚ — **OVERIŤ** | DPA ScrapingBee — **uložiť DPA + overiť lokalitu** | ⚪ zámerne neuvedený v zásadách (verejné inzeráty, nie klientske OÚ) · *env na prode: nie — overí Bezpecnost (Vercel)* | Aleš Machovič + Compliance |
| 10 | **OpenStreetMap / Nominatim** | Geokódovanie adresy nehnuteľnosti pri ukladaní (adresný reťazec) | Veľká Británia | Rozhodnutie o primeranosti (UK) — **verejná služba bez DPA** | **Posúdiť** — buď self-host Nominatim, alebo doložiť poskytovateľa s DPA | ✅ áno (v2.2) | Nehnuteľnosti tech lead + Compliance |
...
Aktuálna privacy policy (`src/app/gdpr/page.tsx`, v2.2) v sekcii 9 „Prenosy mimo EÚ/EHP" uvádza **7 subprocesorov:** Supabase, Vercel, Resend, Google, Anthropic, OpenAI a OpenStreetMap/Nominatim.

**FINÁLNE ROZHODNUTIE (2026-06-07, po potvrdení CEO):**
- ✅ **DO sekcie 9 doplnené (VYKONANÉ, commit 6e64307): OpenAI** (fallback AI na neosobné údaje, USA/DPF) + **Nominatim/OSM**
  (geokódovanie adries nehnuteľností, VB — rozhodnutie o primeranosti UK).
- ❌ **Stripe — NEPRIDÁVAŤ:** CEO potvrdil, že platby predplatného zatiaľ NEBEŽIA (kód pripravený, NEAKTÍVNY na prode). Pridať
  až keď sa billing zapne.
- ❌ **Twilio — NEPRIDÁVAŤ:** CEO rieši OTP **emailom** (= Resend, už v sekcii 9 nepriamo cez Resend),
  SMS cez Twilio sa nepoužíva (kód pripravený, NEAKTÍVNY na prode). (Fast-follow: odstrániť závislosť na sendSms path — vlastník Náberáky.)
...
| P1 | Uložiť DPA pre Stripe a Twilio (do privacy policy doplniť až pri zapnutí na prode) | Compliance (text) + Aleš (súhlas) |
